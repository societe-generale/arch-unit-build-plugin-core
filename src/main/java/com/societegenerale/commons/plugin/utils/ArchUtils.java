...
public class ArchUtils {
	...
	public static JavaClasses importAllClassesInPackage(String path, String classFolder) {
		...
		// not great design, but since all the rules need to call this, it's very
		// convenient to keep this method static

		if (log == null) {
			throw new IllegalStateException("please make sure you instantiate " + ArchUtils.class + " with a proper "
					+ Log.class + " before calling this static method");
		}
		...
		if (classesPath.toFile().exists()) {
			...
			StringBuilder warnMessage = new StringBuilder("classpath ").append(classesPath.toFile())
					.append("doesn't exist : loading all classes from root, ie ").append(path);
